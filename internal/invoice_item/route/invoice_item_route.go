...
import (
	"Back-end/config"
	d "Back-end/database"
	h "Back-end/internal/invoice_item/handler"
	r "Back-end/internal/invoice_item/repository"
	u "Back-end/internal/invoice_item/usecase"
...
func RegisterInvoiceItemGroupAPI(e *echo.Echo, conf config.Config) {
	...
	apiInvoiceItem.GET("/number/:number", hand.GetInvoiceItemsByNumberController, middleware.JWT([]byte(conf.JWT_KEY)))
